...
def list_to_link(lst):
    ...
    if len(lst)==1:
        ...
        return Link(lst[0], list_to_link(lst[1:len(lst)]))
...
def link_to_list(link):
    ...
    []
    """
    return [link[index] for index in range(len(link))]
...
def remove_all(link , value):
    ...
    <0 1>
    """
    list = link_to_list(link)
    list = [val for val in list if val!=value]
    # list = [link[i] for i in range(len(link)) if link[i]!=value]
    link.first = list[0]
    link.rest = list_to_link(list[1:])

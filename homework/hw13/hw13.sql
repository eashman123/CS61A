...
with paths(flight, dest, price_t, n) as (
 SELECT departure || ', ' || arrival, arrival, price, 1 FROM flights WHERE departure = 'SFO' UNION
 SELECT flight || ', ' || arrival, arrival, price_t + price, n+1 FROM paths, flights WHERE n < 2 AND dest = departure)
  SELECT flight, price_t FROM paths WHERE dest = 'PDX' ORDER BY price_t;

CREATE TABLE number_of_options AS
  SELECT COUNT(DISTINCT meat) from main_course;

CREATE TABLE calories AS
  SELECT COUNT(*) FROM main_course, pies WHERE pies.calories + main_course.calories < 2500;

CREATE TABLE healthiest_meats AS
  SELECT meat, MIN(m.calories + p.calories) FROM main_course as m, pies as p GROUP BY meat HAVING MAX (m.calories + p.calories) < 3000;
...
CREATE TABLE shopping_list AS
  SELECT p.name, l.store as store FROM lowest_prices as l, products as p WHERE l.item = p.name GROUP BY p.category HAVING min(p.MSRP/p.rating);

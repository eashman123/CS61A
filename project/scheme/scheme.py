...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Unequal number of parameters and values")
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        def evalate_operands(operand):
            if operand is nil:
                return nil
            return scheme_eval(operand, env)

        return scheme_apply(self, operands.map(evalate_operands), env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Invalid number of parameters")
        # END PROBLEM 4
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame thant binds my formal parameters to ARGS, a Scheme list
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        #scheme_apply(self, complete, env)
        #complete_eval(Thunk(operands, env) )

        #t = Thunk(operands, env)
        #return complete_eval(t)
        #print("HERE:", Thunk(operands, env))
        #return Thunk(operands, env)
        ...
        #return scheme_apply(self, complete_eval(t), env)
        #scheme_apply(self, operands, env)
        #return complete_eval(t)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        env.define(target.first, LambdaProcedure(target.second, 
            expressions.second, env) )
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if not current_val and type(current_val) is bool:
            return current_val
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        current_val = scheme_eval(expressions.first, env)
        if current_val or type(current_val) is not bool:
            return current_val
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def make_symbols_pair(symbols):
        if symbols is nil:
            return nil
        return Pair(symbols.first.first, make_symbols_pair(symbols.second))

    def make_values_pair(values):
        if values is nil:
            return nil
        return Pair(values.first.second.first, make_values_pair(values.second))

    def eval_operands(operand):
        if operand is nil:
            return nil
        return scheme_eval(operand, env)
    ...
    symbols = make_symbols_pair(bindings)
    vals = make_values_pair(bindings)

    check_formals(symbols)
    return env.make_child_frame(symbols, vals.map(eval_operands) )
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            #if first in env.bindings and isinstance(env.bindings[first], MacroProcedure):
                #print("scheme_optimized_eval")
            #    result = scheme_eval(first, env).eval_call(rest, env) #rest.map(lambda y: scheme_eval(y, env)), env)
            #else:
            #result = scheme_eval(first, env).eval_call(rest.map(lambda y: scheme_eval(y, env)), env)
            operator = scheme_eval(first, env)
            check_procedure(operator)

            #return operator.eval_call(rest, env)
            result = operator.eval_call(rest, env)
            #result = scheme_apply(scheme_eval(first, env), rest.map(lambda y: scheme_eval(y, env)), env)
            # END PROBLEM 20
    return result
